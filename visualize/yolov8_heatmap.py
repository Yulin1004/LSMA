...
class yolov8_heatmap:
    ...
    def process(self, img_path,images):
        # img process
    ...
    def __call__(self, img_path,images):
        if os.path.isdir(img_path):
            for img_path_ in os.listdir(img_path):
                self.process(f'{img_path}/{img_path_}',images)
        else:
            self.process(img_path,images)
        
def get_params():
    params = {
        'weight': 'E:\\code\\ultralytics1\\yolov8_co\\cbam_best.pt', # 现在只需要指定权重即可,不需要指定cfg
        'device': 'cuda:0',
...
if __name__ == '__main__':
    ...
    model(r'E:\code\ultralytics1\images\coco_airplane_1.jpg', 'result')